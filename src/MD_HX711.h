...
Strain gauge load cells are commonly used on the corners of bathroom scales 
(and similar applications). Each load cell is a half Wheatstone bridge, as 
shown below.

\image{inline} html HX711_Strain_Gauge_Loadcell.png "Strain Gauge Load cell"

This type of load cell must be connected into a circuit that creates the full
Wheatstone bridge formation, with each leg made up of a strain resistor from 
two of the strain gauge load cells.

\image{inline} html HX711_Combinator_Circuit.jpg "Combinator Board Circuit"

A simple way to do this is to create a Combinator Board where the wires 
from the individual strain gauge cells are combined in a 'hub' arrangement 
to produce the normal 4 Wheatstone Bridge connections.

\image{inline} html HX711_Combinator_Board.jpg "Combinator Board"
____

## Wire Connections

The HX711 module is used to get measurable data out of a load cell and 
strain gauge. Load cells come in various shapes and are rated from 0.1kg 
to over 1000kg full scale.

The HX711 module's A channel interfaces to the load cell through four wires 
labeled E+ (or RED), E- (BLK), A-(WHT), A+(GRN), and an optional shielding 
ground SD(YLW) on the circuit board.
...
E+ and E- are the positive voltage and ground connections to load cell; 
the A+/A-, B+/B- the differential outputs from the load cell.
